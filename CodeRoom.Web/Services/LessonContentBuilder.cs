...
_ when courseTitle.Contains("Python", StringComparison.OrdinalIgnoreCase) => $"This Python lesson focuses on {title.ToLowerInvariant()}. Pay attention to syntax, data flow, error handling and the small verification steps that make a script predictable and reusable.",
        _ when courseTitle.Contains("Bash", StringComparison.OrdinalIgnoreCase) || courseTitle.Contains("Linux", StringComparison.OrdinalIgnoreCase) => $"This Linux and shell lesson focuses on {title.ToLowerInvariant()}. Connect each command to the system state it changes or observes, and practise safely with a test file, process or directory.",
        _ when courseTitle.Contains("AWS", StringComparison.OrdinalIgnoreCase) || courseTitle.Contains("Azure", StringComparison.OrdinalIgnoreCase) || courseTitle.Contains("Cloud", StringComparison.OrdinalIgnoreCase) => $"This cloud lesson focuses on {title.ToLowerInvariant()}. Compare the architectural goal, access boundary, operational responsibility and security trade-off before choosing a service or control.",
        _ when courseTitle.Contains("SOC", StringComparison.OrdinalIgnoreCase) => $"This SOC lesson focuses on {title.ToLowerInvariant()}. Build an evidence-based timeline, distinguish observations from assumptions and identify the next data source that would reduce uncertainty.",
        _ when courseTitle.Contains("Security", StringComparison.OrdinalIgnoreCase) || courseTitle.Contains("Cyber", StringComparison.OrdinalIgnoreCase) => $"This security lesson focuses on {title.ToLowerInvariant()}. Think in terms of attack surface, trust boundaries, defensive controls, observable evidence and the least-privilege action that reduces risk.",
        _ when courseTitle.Contains("Networking", StringComparison.OrdinalIgnoreCase) || courseTitle.Contains("CCNA", StringComparison.OrdinalIgnoreCase) => $"This networking lesson focuses on {title.ToLowerInvariant()}. Draw the traffic path, identify the address or protocol involved, and verify your reasoning with a concrete packet, command or topology example.",
        _ when courseTitle.Contains("SQL", StringComparison.OrdinalIgnoreCase) || courseTitle.Contains("MySQL", StringComparison.OrdinalIgnoreCase) || courseTitle.Contains("Database", StringComparison.OrdinalIgnoreCase) => $"This database lesson focuses on {title.ToLowerInvariant()}. Relate the concept to tables, keys, constraints and the query or application operation that would use it.",
        _ => "        _ => $\"This topic explains an important part of {courseTitle}. Focus on the terminology, the purpose of the concept, the conditions in which it is used and the mistakes that commonly lead to incorrect results.\""
    };
...
_ when title.Contains("Automation", StringComparison.OrdinalIgnoreCase) => "Automate one repetitive step, add a safe input check, log the result and test one failure case.",
        _ when title.Contains("Cloud", StringComparison.OrdinalIgnoreCase) || title.Contains("IAM", StringComparison.OrdinalIgnoreCase) || title.Contains("VPC", StringComparison.OrdinalIgnoreCase) || title.Contains("Azure", StringComparison.OrdinalIgnoreCase) => "Take a small workload and map identity, network boundary, compute/storage choice and one monitoring signal before selecting the service.",
        _ when title.Contains("Log", StringComparison.OrdinalIgnoreCase) || title.Contains("SOC", StringComparison.OrdinalIgnoreCase) || title.Contains("Monitoring", StringComparison.OrdinalIgnoreCase) => "Build a mini timeline from a sample event and list one fact, one hypothesis and one additional evidence source.",
        _ when title.Contains("Bash", StringComparison.OrdinalIgnoreCase) || title.Contains("Shell", StringComparison.OrdinalIgnoreCase) => "Run the command safely in a test location, capture its output, then repeat it with an edge case such as an empty input or missing file.",
        _ when title.Contains("Networking", StringComparison.OrdinalIgnoreCase) || title.Contains("Routing", StringComparison.OrdinalIgnoreCase) || title.Contains("Subnet", StringComparison.OrdinalIgnoreCase) => "Draw the packet path or subnet range first, then verify it using a small CLI command or worked example.",
        _ when title.Contains("Security", StringComparison.OrdinalIgnoreCase) || title.Contains("XSS", StringComparison.OrdinalIgnoreCase) || title.Contains("CSRF", StringComparison.OrdinalIgnoreCase) || title.Contains("SQL injection", StringComparison.OrdinalIgnoreCase) => "Identify the untrusted input and trust boundary, then name the specific server-side control that should protect it.",
        _ when title.Contains("Database", StringComparison.OrdinalIgnoreCase) || title.Contains("SQL", StringComparison.OrdinalIgnoreCase) => "Create a tiny schema or query, inspect the result, then deliberately test a missing row, duplicate value or invalid condition.",
        _ => "        _ => \"Create a tiny example that demonstrates the idea, test it with a normal case and one edge case, and write down what you learned from the result.\""
    };
}
